...
public class WeChat {
    ...
    public static final String KEY = "$WECHAT_TOKEN";
     ...
     * @throws Exception
     */
    public static String getAccessToken(SpyMemcachedClient memcachedClient) throws Exception {
...
//	        String secret = ConfKit.get("AppSecret");
			...
			if (StringUtils.isNotBlank(accessToken)) {// 保留2小时
				memcachedClient.set(KEY, 1000 * 60 * 60 * 2, accessToken);
			}
		}
...
//
     ...
     * @throws Exception
     */
    public static boolean payfeedback(String openid, String feedbackid, SpyMemcachedClient client) throws Exception {
     ...
     * @return
     */
    public static String processing(String responseInputString, ApplicationContext ctx) {
        InMessage inMessage = parsingInMessage(responseInputString);
        OutMessage oms = null;
        String xml = "";
        try {
			MessageProcessingHandler messageProcessingHandler = null;
			if (ctx != null) {
				messageProcessingHandler = ctx.getBean(MessageProcessingHandler.class);
			} else {
				// 加载处理器
		        if (messageProcessingHandlerClazz == null) {
		            // 获取自定消息处理器，如果自定义处理器则使用默认处理器。
//		            String handler = ConfKit.get("MessageProcessingHandlerImpl");
		        	String handler = Global.getConfig("MessageProcessingHandlerImpl");
				...
				messageProcessingHandler = (MessageProcessingHandler) messageProcessingHandlerClazz.newInstance();
			}
            //取得消息类型
