...
public class WeChatPayNotifyServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		SpyMemcachedClient memcachedClient = SpringContextHolder.getBean(SpyMemcachedClient.class);
		try {
			...
			boolean temp = WxPay.verifySign(timestamp, noncestr, openid, issubscribe, appsignature);
			if (!temp) {
		...
		try {
			...
			//发货通知！推荐支付成功后就发送此通知...
			WxPay.delivernotify(accessToken, openid, transId, transId);
		} catch (Exception e) {
			e.printStackTrace();
		}
