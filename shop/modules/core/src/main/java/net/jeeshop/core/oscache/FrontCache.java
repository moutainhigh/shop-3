...
//		}
	...
	public void loadPlugConfig() {
		...
		AlipayConfig.key = pay.getKeyword();
...
//		logger.info("SystemManager.alipayConfig="+SystemManager.alipayConfig);
	...
	public void loadAttributeList() {
		...
		if(attrs!=null && attrs.size()>0){
			...
			if(map.size()>0){
				for(Iterator<Entry<Long, Attribute>> it = SystemManager.attrsMap.entrySet().iterator();it.hasNext();){
...
//	}
	...
	public static Long getPid(Long catalogID){
		if(catalogID == null){
			throw new NullPointerException();
		}
		Catalog catalog = SystemManager.catalogsMap.get(catalogID);
...
//		if(item.getPid()==0L&& (item.getChildren()==null || item.getChildren().size()==0)){
//			return;
//		}
//		
...
//		SystemManager.noticeList = noticeService.selectList(notice);
	...
	public void loadProductStock() {
		...
		synchronized (SystemManager.product_stock_lock) {
			if(SystemManager.productStockMap==null){
				SystemManager.productStockMap = new ConcurrentHashMap<Long, ProductStockInfo>(list.size());
			}else{
	...
	public void loadActivityScoreProductList(){
		...
		for(Iterator<Entry<Long, Activity>> it = SystemManager.activityMap.entrySet().iterator();it.hasNext();){
		...
		if(list!=null){
			for(int i=0;i<list.size();i++){
				...
				if(!activity.isExpire()){
					//计算活动多久结束，是否已结束
					activity.setActivityEndDateTime(activity.getEndDate());					
				}
