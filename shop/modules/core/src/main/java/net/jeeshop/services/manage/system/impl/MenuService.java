...
public class MenuService implements Services<Menu> {
	...
	public void deletes(String ids, String deleteParent) {
		...
		Long[] idArr = (Long[])ConvertUtils.convert(StringUtils.split(ids,","), Long.class);//ids.split(",");
//		System.out.println(Arrays.toString(idArr));
		// 按照从小到大排序
		Arrays.sort(idArr, new Comparator<Long>() {
...
//							System.out.println("del menus:" + menu);
	 ...
	 */
	private boolean checkAllContains(Long[] idArr, List<Menu> list) {
		int n = list.size();
	...
	// 加载根节点
	public List<MenuItem> loadMenus(User u, Long pid, String url) {
...
//			Menu entry = it.next();
	...
	private void loadChildrenByPid(MenuItem item, Menu menu, String url, User u) {
		...
		for (int i = 0; i < item.getChildren().size(); i++) {
			...
			itemMenu.setPid(new Long(item.getChildren().get(i).getId()));
	...
	public Menu selectById(Long id) {
		return (Menu) dao.selectOne("menu.selectById", id);
	}
}
