...
<body>
				...
				<div class="row">
					...
					<table class="table table-bordered">
						...
						<tr>
							...
							<td>
						    	<%
						    	application.setAttribute("topArea", SystemManager.topArea);
						    	%>
						    	<s:select list="#application.topArea.children" listKey="code" listValue="name" onchange="changeProvince()"
						    	...
						    	<s:else>
						    		<s:iterator value="#application.topArea.children" status="i" var="item">
						    			<s:if test="#item.code==e.ordership.provinceCode">
						    				<s:select list="#item.children" listKey="code" listValue="name"  id="citySelect" name="e.ordership.cityCode" onchange="changeCity()" 
							    				headerKey="" headerValue="--选择城市--"   cssClass="form-control" />
						    			</s:if>
						    		</s:iterator>
					    		</s:else>
	...
	<%-- 					    			code=<s:property value="value.code"/> --%>
						    			<s:if test="#item.code==e.ordership.provinceCode">
						    				<s:iterator value="#item.children" var="item1">
								    			<s:if test="#item1.code==e.ordership.cityCode">
								    				 <s:select list="#item1.children"  id="areaSelect" name="e.ordership.areaCode" listKey="code" listValue="name" 
						    							headerKey="" headerValue="--选择区域--" cssClass="form-control" />
								    			</s:if>
							    			</s:iterator>
						    			</s:if>
						    		</s:iterator>
