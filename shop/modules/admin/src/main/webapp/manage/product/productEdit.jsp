<%@page import="com.tuisitang.modules.shop.entity.Catalog"%>
...
<link href="<%=request.getContextPath() %>/resource/select2/select2.css" type="text/css" rel="stylesheet"/>
<script src="<%=request.getContextPath() %>/resource/select2/select2.js" type="text/javascript"></script>	
<script src="<%=request.getContextPath() %>/resource/select2/select2_locale_zh-CN.js" type="text/javascript"></script>	
<style>
#insertOrUpdateMsg{
...
<s:form action="product" id="form" name="form" namespace="/manage" theme="simple" enctype="multipart/form-data" method="post">
	...
	<input type="hidden" value="<s:property value="e.selectCatalogIds"/>" id="selectCatalogId"/>
...
<!-- 			<a href="product!selectList.action?init=y" class="btn btn-inverse">返回</a> -->
				...
				<table class="table table-condensed">
							...
							<tr>
								...
								<td colspan="1">
									...
									%>
									<select onchange="catalogChange(this)" name="e.selectCatalogIds" id="catalogSelect" multiple="multiple" style="width:400px" data-rule="required;selectCatalogIds">
										<option></option>
		...
		<!-- 					</tr> -->
							...
							<tr>
								...
								<td style="text-align: left;">
									<s:select list="#{'n':'否','y':'是'}" id="pid" name="e.isRecommend" 
										listKey="key" listValue="value"  />
								</td>
					...
					<table class="table table-bordered">
						<tr>
							<td colspan="11">
								...
								<button method="product!deleteImageByImgPaths.action" onclick="return deleteImageByImgPaths(); class="btn btn-success">
				...
				<table class="table">
					...
					<s:if test="e.specList!=null and 1==1">
						<s:iterator value="e.specList" var="item" status="stat">
							<tr>
								...
								<th><s:textfield name="e.specList[%{#stat.index}].specType" cssClass="search-query input-small"/></th>
					...
					<s:else>
						<%for(int i=0;i<10;i++){ %>
							<tr>
								<th style="display: none;"><s:hidden name="id"/></th>
								<th><s:textfield name="e.specArray.specType" cssClass="search-query input-small"/></th>
...
function selectDefaultCatalog(){
	var _catalogHidden = $("#selectCatalogId").val();
	if(_catalogHidden != null && _catalogHidden != ''){
		var catalogIds = _catalogHidden.split(",");
		$("#catalogSelect").select2("val", catalogIds);
...
KindEditor.ready(function(K) {
	var editor = K.editor({
		fileManagerJson : '<%=request.getContextPath() %>/resource/kindeditor-4.1.7/jsp/file_manager_json.jsp?prefix=${e.id}'
	});
