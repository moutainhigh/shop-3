...
<%-- 					<li><a href="#tabs-6"><strong>系统使用帮助</strong></a></li> --%>
				...
				<div id="tabs-1">
					...
					<table class="table table-bordered">
						<tr>
							<td>未付款订单数：<a href="order!selectList.action?init=y&paystatus=n&status=init"><%=orderReport.getNotPayCount()%></a></td>
							<td>已付款，但未审核订单数：<a class="Afont22" style="color: #f50;" href="order!selectList.action?init=y&paystatus=y&status=init"><%=orderReport.getPayButNotPassCount()%></a></td>
						</tr>
						<tr>
							<td>已取消订单数：<a href="order!selectList.action?init=y&status=cancel"><%=orderReport.getCancelCount() %></a></td>
							<td>等待发货订单数：<a class="Afont22" style="color: #f50;" href="order!selectList.action?init=y&paystatus=y&status=pass"><%=orderReport.getWaitSendGoodsCount()%></a></td>
						</tr>
					</table>
					...
					<table class="table table-bordered">
						<tr>
							<td>【卖家需立即处理】退款协议等待卖家确认中：
								<a href="order!selectList.action?init=y&refundStatus=WAIT_SELLER_AGREE"><%=orderReport.getWAIT_SELLER_AGREE() %></a>
							</td>
						</tr>
						<tr>
							<td>【卖家需立即处理】等待卖家收货：
								<a href="order!selectList.action?init=y&refundStatus=WAIT_SELLER_CONFIRM_GOODS"><%=orderReport.getWAIT_SELLER_CONFIRM_GOODS() %></a>
							</td>
						</tr>
						...
						<tr>
							<td>【等待买家处理完】卖家不同意协议，等待买家修改：
								<a href="order!selectList.action?init=y&refundStatus=SELLER_REFUSE_BUYER"><%=orderReport.getSELLER_REFUSE_BUYER() %></a>
							</td>
						</tr>
						<tr>
							<td>【等待买家处理完】退款协议达成，等待买家退货：
								<a href="order!selectList.action?init=y&refundStatus=WAIT_BUYER_RETURN_GOODS"><%=orderReport.getWAIT_BUYER_RETURN_GOODS() %></a>
							</td>
						</tr>
					</table>
					...
					<table class="table table-bordered">
						<tr>
							<td>缺货商品数：<a href="product!selectList.action?init=y&selectOutOfStockProduct=true"><%=orderReport.getOutOfStockProductCount()%></a></td>
							<td>未回复的吐槽评论数：<a href="comment!selectList.action?init=y&selectCommentFromIndex=true"><%=orderReport.getNotReplyCommentCount()%></a></td>
						</tr>
					</table>
				</div>
				<div id="tabs-2">
